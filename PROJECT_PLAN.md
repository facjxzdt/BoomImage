...
> 面向单用户、单机 Docker 部署的高性能图床。支持自动生成 AVIF、WebP 等图片变体，并为未来扩展对象存储和 CDN 保留清晰边界。
...
- 当前完整回归为 20 项服务端测试，前后端类型检查、前后端生产构建、备份命令实跑均通过。
...
| 文件存储 | 本地文件系统 | 图片二进制不存入 SQLite |
| 管理界面 | React + Vite | 构建后作为静态资源发布 |
...
4. Caddy 直接读取已生成文件，普通图片请求不经过应用。
5. 文件采用内容哈希寻址，并设置长期不可变缓存。
